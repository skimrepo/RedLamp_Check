"""
Trains one leave-one-out model per (UCR entity, seed) in --entities x
--seeds (default: the 8 PowerDemand entities x seeds 0/1/2), using
core_clustering.online_cli UNMODIFIED -- each run points at the merged
pool build_ucr_anomsim_pool.py already built (AnomSim_v1's 144 entities +
all 8 UCR entities) and excludes just ONE UCR entity via
--exclude_entity_dirs_file, so the resulting model is trained on
AnomSim_v1 + the OTHER 7 UCR entities, having never seen the held-out one.

Multiple seeds exist purely to get a mean +/- across independent training
runs (different weight init / data split) for score_ucr_anomsim_loo.py to
average over -- entities are still leave-one-out per seed, seeds don't
change which entities are held out.
...
many processes fight over one GPU's memory. 8 entities x 3 seeds x ~20 min
~= 8 hours total; --entities/--seeds let you split this across multiple
manual invocations (e.g. one shard per terminal) if you want to risk
running a couple concurrently on a GPU with enough free memory margin.

Resumable: online_cli.py itself skips retraining if output_dir/run_id/
bestmodel.pkl already exists (pass --force to override) -- so re-running
this same command after an interruption just picks up where it left off.
"""
import argparse
import os
import subprocess
    ...
    for seed in args.seeds:
        for entity in args.entities:
            ...
            if not os.path.isfile(exclude_file):
                print(f'[skip] {entity}/seed{seed}: no exclude file at {exclude_file} -- run build_ucr_anomsim_pool.py first')
                continue
            run_id = f'without_{entity_dir_name}_seed{seed}'
